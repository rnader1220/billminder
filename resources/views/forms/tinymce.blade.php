{{-- resources/views/forms/tinymce.blade.php --}}
...
<div class="mb-4">
    ...
    <textarea name="{{ $name }}" id="{{ $name }}" rows="{{ $rows }}" @if($title !='' ) title="{{ $title }}" @endif
        @if($disabled) disabled @endif @if($required) required @endif
        class="form-element tinymce">{{ old($name, $value) }}</textarea>
    @if($helptext != '')
