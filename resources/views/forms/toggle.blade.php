...
<div class="mb-4">
    ...
    <label class="inline-flex relative items-center cursor-pointer" @if($title) title="{{ $title }}" @endif>
        <input type="checkbox" name="{{ $name }}" id="{{ $name }}" value="1" class="sr-only peer" @if(old($name,
            $checked)) checked @endif @if($disabled) disabled @endif>
        <div
        ...
        <div class="dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition peer-checked:translate-x-full">
        </div>
    </label>
